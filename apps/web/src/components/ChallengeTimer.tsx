...
export function ChallengeTimer({ remaining, total }: { remaining: number; total: number }) {
  ...
  return (
    <div className="my-7 mb-8" aria-live="polite">
      <div className="mb-[9px] flex items-end justify-between"><span className="text-[9px] font-extrabold tracking-[0.15em] text-[#656565]">TID KVAR PÅ UPPDRAGET</span><strong className={`${barlow} text-[23px] ${toneStyles[tone].text}`}>{Math.max(0, Math.ceil(remaining))} SEK</strong></div>
