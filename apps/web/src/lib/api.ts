import type { AdminGame, AnswerResult, Challenge, GameSummary, LeaderboardEntry, Session } from '../types'
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    const payload = (await response.json().catch(() => null)) as { message?: string; detail?: string; title?: string } | null
    throw new ApiError(payload?.message ?? payload?.detail ?? payload?.title ?? 'Något gick fel. Försök igen.', response.status)
  }
...
async function protectedRequest<T>(path: string, init: RequestInit) {
  ...
  return request<T>(path, { ...init, headers: { ...init.headers, 'X-CSRF-TOKEN': csrfToken } })
}
...
export const api = {
  ...
  login: (username: string, password: string) => protectedRequest<{ username: string }>('/api/auth/login', { method: 'POST', body: JSON.stringify({ username, password }) }),
  logout: () => protectedRequest<void>('/api/auth/logout', { method: 'POST' }),
  adminGames: () => request<AdminGame[]>('/api/admin/games'),
  updateGame: (game: AdminGame) => protectedRequest<AdminGame>(`/api/admin/games/${game.id}`, { method: 'PUT', body: JSON.stringify(game) }),
  uploadChallengeImage: (challengeId: string, image: File) => {
    const form = new FormData()
    form.append('image', image)
    return protectedRequest<{ imagePath: string }>(`/api/admin/challenges/${challengeId}/image`, { method: 'POST', body: form })
  },
}
