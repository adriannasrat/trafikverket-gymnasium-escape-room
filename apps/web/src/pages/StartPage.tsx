...
import { barlow, eventFooter, eyebrow, fieldLabel, focusRing, formError, primaryButton } from "../uiStyles";
    ...
    <div className="min-h-screen overflow-hidden bg-white">
      ...
      <main className="relative mx-auto flex min-h-[calc(100vh-126px)] w-[min(1180px,100%)] flex-col items-center px-8 pt-[clamp(54px,7vh,88px)] pb-[34px] max-[720px]:min-h-[calc(100vh-120px)] max-[720px]:px-[18px] max-[720px]:pt-[43px] max-[720px]:pb-7">
        ...
        <section className="relative z-2 w-[min(720px,100%)] text-center">
          <p className={`${eyebrow} mb-5 justify-center`}>TRAFIKVERKET · ESCAPE ROOM</p>
          <h1 className={`${barlow} m-0 text-[clamp(64px,7vw,92px)] leading-[.86] font-bold tracking-[-.025em] text-[#202020] uppercase max-[720px]:text-[57px] max-[420px]:text-[50px]`}>
            Håll Sverige
            <br />
          ...
          <form className="mx-auto mt-[31px] w-[min(640px,100%)] text-left" onSubmit={start}>
            <label className={`${fieldLabel} mb-2`} htmlFor="playerName">DITT NAMN</label>
          ...
          {game && (
            <p className="mt-[18px] inline-flex items-center gap-[9px] text-[10px] text-[#606060] max-[420px]:flex-col max-[420px]:items-start max-[420px]:gap-1">
              <span className="text-[8px] font-extrabold tracking-[.13em] text-[#d70000]">AKTIVT UPPDRAG</span>
              {game.title}
            </p>
          )}
        </section>
        ...
        >
          <header className="mb-[11px] flex items-end justify-between gap-[18px]">
            <div>
              ...
              <h2 className={`${barlow} m-0 text-[26px] text-[#202020] uppercase`} id="best-times-title">Bästa tider</h2>
            </div>
            <Link className={`flex items-center gap-[6px] text-[10px] font-bold text-[#555] ${focusRing}`} to="/leaderboard">
              Visa alla <ArrowRight size={14} />
            </Link>
          </header>
          {leaders === null ? (
            <div className="flex min-h-12 items-center justify-center gap-[10px] bg-[#f5f5f5] text-[10px] text-[#6c6c6c]">Hämtar tider…</div>
          ) : leaders.length === 0 ? (
            <div className="flex min-h-12 items-center justify-center gap-[10px] bg-[#f5f5f5] text-[10px] text-[#6c6c6c]">
              ...
              <span className="flex gap-1 max-[720px]:grid">
                <strong className="text-[#333]">Första rekordet väntar.</strong> Din tid kan bli den
                första.
              </span>
            </div>
          ) : (
            <ol className="m-0 grid list-none grid-cols-3 gap-2 p-0 max-[720px]:grid-cols-1">
              {leaders.map((entry) => (
                <li className="grid grid-cols-[25px_1fr] items-center gap-x-2 gap-y-px bg-[#f5f5f5] px-3 py-[10px] text-left" key={entry.id}>
                  <span className={`${barlow} row-span-2 grid size-6 place-items-center bg-[#d70000] font-bold text-white`}>{entry.rank}</span>
                  <strong className="overflow-hidden text-[11px] text-ellipsis whitespace-nowrap text-[#303030]">{entry.playerName}</strong>
                  <time className={`${barlow} text-[13px] font-semibold tracking-[.04em] text-[#686868]`}>{formatElapsed(entry.elapsedMilliseconds)}</time>
                </li>
              ))}
            </ol>
          )}
      ...
      <footer className={eventFooter}>
        ...
        <nav className="flex gap-[25px]">
          <Link className={`flex items-center gap-[5px] text-[#555] ${focusRing}`} to="/leaderboard">Topplista</Link>
          <Link className={`flex items-center gap-[5px] text-[#555] ${focusRing}`} to="/admin/login">
